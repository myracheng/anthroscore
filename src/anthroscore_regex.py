    ...
    for i, k in df.iterrows():
        ...
        if text.strip():
            ...
            for _parsed_sentence in doc.sents:
                target = re.sub('|'.join([_pattern.lower() for _pattern in pattern_list]), '<mask>', str(_parsed_sentence),flags=re.I)
                if target != str(_parsed_sentence):
                    final.append((str(_parsed_sentence), target, text_id, '','','|'.join([_pattern.lower() for _pattern in pattern_list]),str(_parsed_sentence).lower()))
